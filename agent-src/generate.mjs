 ...
 */
function createScriptedPrompter(values) {
  const lookup = (key) => {
    let cur = values;
 ...
 */
async function runInterview(prompter, { detectRepoSlug, projectRoot }) {
 ...
 */
async function cmdInit(projectRoot, { prompter } = {}) {
  const dest = path.join(projectRoot, 'ai-project.json');
  let active = prompter;
  if (!active) {
...
export {
  buildGlobalTokens, loadConfig, kebabCase, parseOriginSlug, azureMapping, buildProjectConfig,
  renderSetupDoc, cmdScaffold, renderAll, cmdInit, createScriptedPrompter, runInterview,
};
