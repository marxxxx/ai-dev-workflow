...
- A developer that wrote its handoff into the `{{artifact.journal}}` comment is finished. Do not `followup_task` it and do not raise its context limit — `spawn_agent` a fresh one with `fork_turns: "none"`, incrementing the continuation and keeping the same iteration number.
- The continuation's packet carries the `{{artifact.journal}}` comment id, the remaining criteria, and the continuation number. It reads that comment by id before exploring; that map is what keeps the fresh window from being spent on rediscovery.
- If a developer returns prematurely **without** writing a handoff into the journal, spawn a fresh replacement at the same iteration and continuation number, and tell it in the packet that no handoff exists so it recovers from the branch diff and ticket comments instead.
